...
bool ctrlPgm(std::string v_TEXT)
{
	...
	std::filesystem::path p(v_TEXT.c_str());
											...
											{
												case  strswitch("Pmaitre")		    : b_pgm =true;		break;
                                                case  strswitch("Pecho")		: b_pgm =true;		break;
											}
	return b_pgm;
}
...
gchar *arg_1[] =  { (gchar*)WORKPGM,  NULL};                        /// exemple P_Pgm (argv[1]) = ./programme
gchar *arg_2[] =  { (gchar*) argv[1], NULL};	                    /// arg[1] P_pgm


gchar *arg_3[] =  { (gchar*)(gchar*) argv[1] ,(gchar*) argv[1],(gchar*)argv[2], NULL};		/// arg[1] P_pgm , [2] Pid_msgq
	if (argc == 1 )  {
    ...
    //printf("\ndir %s",(const char*)getcwd(NULL, 0));
