...
const ziglyph = @import("ziglyph/ziglyph.zig");
/// fork https://github.com/JakubSzark/zig-string
/// delete truncate
/// new normalize
/// new StringTo
/// new findPos
...
pub const String = struct {
    ...
    /// fromString   clear and insert string
    pub fn StringTo(self: *String, char: []const u8) Error!void {
        self.clear();
        try self.insert(char, self.len());
    }
    ...
    /// Removes the last character form End ===> think about sql
    pub fn normalize(self: *String, end: usize) void {
        ...
        if (self.buffer) |buffer| {
            ...
            while (i < self.size) {
                const size = String.getUTF8Size(buffer[i]);
                if (i + size > self.size or m >= end ) break;
                i += size;
                m += 1 ;
            }
            self.size = i;
    ...
    pub fn findPos(self: String, pos: usize , literal: []const u8) ?usize {
        if (pos <= self.size) {
            if (self.buffer) |buffer| {
                const index = std.mem.indexOf(u8, buffer[pos..self.size], literal);
                if (index) |i| {
                    return String.getIndex(buffer, i + pos, false);
                }
            }
        }
        return null;
    ...
    pub fn cloneDef(self: String) Error!String {
        var newString = String.init(self.allocator);
        newString.clear();
        return newString;
    }
    ...
    pub fn toLowerStr(self: *String) !void {
        if (self.len() != 0) {
            const test_allocator = std.testing.allocator;
            try self.StringTo(try ziglyph.toLowerStr(test_allocator , self.str()));
        }
    }

    /// Converts all (ASCII) uppercase letters to lowercase
    pub fn toUpperStr(self: *String) !void {
        if (self.len() != 0) {
            const test_allocator = std.testing.allocator;
            try self.StringTo(try ziglyph.toUpperStr(test_allocator , self.str()));
        }
    }
